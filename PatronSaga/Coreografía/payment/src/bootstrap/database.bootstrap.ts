...
export class DatabaseBootstrap implements Initialize {
	async initialize(): Promise<any> {
		const promiseInitialize = new Promise((resolve, reject) => {
			...
			const connectionString = `mongodb://${env.DATABASE.MONGO.USER}:${env.DATABASE.MONGO.PASS}@${env.DATABASE.MONGO.HOST}/${env.DATABASE.MONGO.DB}?authSource=${env.DATABASE.MONGO.AUTH}&retryWrites=true&w=majority`;
			const options = {
